...
export class ContaOperadoresService {
  constructor(@InjectRepository(ContaOperadoresEntity) private readonly contaOperadoresRepository: Repository<ContaOperadoresEntity>) {}

  async getAllOperadores(): Promise<ContaOperadoresQueryResponse> {
    try {
      return new Promise<ContaOperadoresQueryResponse>(resolve => {
        this.contaOperadoresRepository
          .find()
          .then(result => {
            resolve({
              success: true,
