...
export class DwhConexionesResolver {
    ...
    async getDWHConexion(
        @Args({ name: 'idDivision', type: () => Int}) idDivision: number
    ): Promise<DWHConexionQueryResponse> {
        return this._dwhConexionesService.getDWHConexion(idDivision);
    }

    @Mutation(() =>  MutationResponse)
    async updateDWhConexion(
        @Args({ name: 'dwhConexionInput', type: () => DWHConexionesInput }) dwhConexionInput: DWHConexionesInput
