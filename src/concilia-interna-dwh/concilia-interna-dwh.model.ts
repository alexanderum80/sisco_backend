...
UNION ALL
    SELECT '', ESIR.Fecha, ESIR.IdCuentaFinanciera AS Emisor, GAM.IdGerencia AS Receptor, LTRIM(LEFT(ESIR.Documento, CASE CHARINDEX('!!', ESIR.Documento) WHEN 0 THEN CHARINDEX('!!', ESIR.Documento) ELSE CHARINDEX('!!', ESIR.Documento) - 1 END)) AS Documento, 0 AS ImporteE, SUM(ISNULL(ESID.Cantidad, 0) * ISNULL(ESID.PCosto, 0)) AS ImporteR
