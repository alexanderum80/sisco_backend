...
export const queryConciliaInternaDWH = `SELECT Emisor, MAX(T.FechaE) AS FechaE, Receptor, MAX(T.FechaR) AS FechaR, Documento, ROUND(SUM(ISNULL(ImporteE, 0)) * -1, 2) AS ImporteE, ROUND(SUM(ISNULL(ImporteR, 0)), 2) AS ImporteR, ROUND(SUM(ISNULL(ImporteE, 0) * -1 - ISNULL(ImporteR, 0)), 2) AS Diferencia
    FROM (SELECT ESIR.Fecha AS FechaE, '' AS FechaR, GAM.IdGerencia AS Emisor, ESIR.IdCuentaFinanciera AS Receptor, LTRIM(LEFT(ESIR.Documento, CASE CHARINDEX('!!', ESIR.Documento) WHEN 0 THEN CHARINDEX('!!', ESIR.Documento) ELSE CHARINDEX('!!', ESIR.Documento) - 1 END)) AS Documento, SUM(ISNULL(ESID.Cantidad, 0) * ISNULL(ESID.PCosto, 0)) AS ImporteE, 0 AS ImporteR
    FROM EntradasSalidasInternasD AS ESID INNER JOIN EntradasSalidasInternasR AS ESIR ON ESID.idOperacion = ESIR.idOperacion INNER JOIN Gerencia_Ano_Mes AS GAM ON ESIR.IdGAM = GAM.IdGAM INNER JOIN UnidadesComerciales.dbo.UnidadesComerciales AS UCE ON UCE.IdUnidad = GAM.IdGerencia INNER JOIN UnidadesComerciales.dbo.Subdivisiones AS SubDE ON SubDE.IdSubdivision = UCE.IdSubdivision INNER JOIN UnidadesComerciales.dbo.Divisiones AS DivE ON SubDE.IdDivision = DivE.IdDivision INNER JOIN UnidadesComerciales.dbo.UnidadesComerciales AS UCR ON UCR.IdUnidad = ESIR.IdCuentaFinanciera INNER JOIN UnidadesComerciales.dbo.SubDivisiones AS SubDR ON SubDR.IdSubdivision = UCR.IdSubdivision INNER JOIN UnidadesComerciales.dbo.Divisiones AS DivR ON DivR.IdDivision = SubDR.IdDivision
...
UNION ALL
    SELECT '', ESIR.Fecha, ESIR.IdCuentaFinanciera AS Emisor, GAM.IdGerencia AS Receptor, LTRIM(LEFT(ESIR.Documento, CASE CHARINDEX('!!', ESIR.Documento) WHEN 0 THEN CHARINDEX('!!', ESIR.Documento) ELSE CHARINDEX('!!', ESIR.Documento) - 1 END)) AS Documento, 0 AS ImporteE, (SUM(ISNULL(ESID.Cantidad, 0) * ISNULL(ESID.PCosto, 0))) - (SUM((ISNULL(ESID.Cantidad, 0) - CASE isnull(ESID.CantidadOficial, 0) WHEN 0 THEN isnull(ESID.Cantidad, 0) ELSE isnull(ESID.CantidadOficial, 0) END) * (ISNULL(ESID.PCosto, 0)))) AS ImporteR
    FROM EntradasSalidasInternasD AS ESID INNER JOIN EntradasSalidasInternasR AS ESIR ON ESID.idOperacion = ESIR.idOperacion INNER JOIN Gerencia_Ano_Mes AS GAM ON ESIR.IdGAM = GAM.IdGAM INNER JOIN UnidadesComerciales.dbo.UnidadesComerciales AS UCE ON UCE.IdUnidad = ESIR.IdCuentaFinanciera INNER JOIN UnidadesComerciales.dbo.Subdivisiones AS SubDE ON SubDE.IdSubdivision = UCE.IdSubdivision INNER JOIN UnidadesComerciales.dbo.Divisiones AS DivE ON SubDE.IdDivision = DivE.IdDivision INNER JOIN UnidadesComerciales.dbo.UnidadesComerciales AS UCR ON UCR.IdUnidad = GAM.IdGerencia INNER JOIN UnidadesComerciales.dbo.SubDivisiones AS SubDR ON SubDR.IdSubdivision = UCR.IdSubdivision INNER JOIN UnidadesComerciales.dbo.Divisiones AS DivR ON DivR.IdDivision = SubDR.IdDivision
