...
export class ContaComprobarExpresiones {
  ...
  @ManyToOne(() => ContaExpresionesResumen, contaExpresionesResumen => contaExpresionesResumen.contaComprobarExpresiones, { onDelete: 'CASCADE', onUpdate: 'CASCADE' })
  @JoinColumn([{ name: 'IdExpresion', referencedColumnName: 'idExpresion' }])
  idExpresion: ContaExpresionesResumen;

  @ManyToOne(() => ContaOperadores, contaOperadores => contaOperadores.contaComprobarExpresiones)
  @JoinColumn([{ name: 'IdOperador', referencedColumnName: 'id' }])
  idOperador: ContaOperadores;

  @ManyToOne(() => ContaExpresionesResumen, contaExpresionesResumen => contaExpresionesResumen.contaComprobarExpresiones2)
